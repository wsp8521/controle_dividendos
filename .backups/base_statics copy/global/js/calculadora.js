...
export function atualizarSaldo() {
    ...
    document.querySelectorAll("tbody tr").forEach((row) => {
        const qtdEl = row.querySelector('[data-field="qtd_calc"]');
        const cotacaoText = row.cells[2]?.textContent?.replace("R$", "").replace(/\./g, "").replace(",", ".") || "0";
        const cotacao = parseFloat(cotacaoText);

        const qtd = parseFloat(qtdEl?.innerText?.replace(",", ".") || "0");

        const total = qtd * cotacao;
        totalGeral += total;

        const totalAtivoEl = row.querySelector(".total-ativo");
        if (totalAtivoEl) {
            totalAtivoEl.textContent = formatCurrency(total);
        }
    });

    // Captura valor do investimento (campo no topo)
    const investimentoEl = document.querySelector('[data-field="valor_investimento"]');
    const investimentoText = investimentoEl?.textContent?.replace("R$", "").replace(/\./g, "").replace(",", ".") || "0";
    const investimento = parseFloat(investimentoText);

    const saldo = investimento - totalGeral;
